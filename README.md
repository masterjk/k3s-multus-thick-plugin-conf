...
├── bridge                    [existing symlink, unchanged]
...
### 📁 Multus Runtime Directories

```
/run/multus/                  [NEW - created by Multus daemon]
├── multus-cni-network.sock   [NEW - Unix socket for Multus]
└── (other runtime files)

/var/lib/cni/multus/          [NEW - created by Multus daemon]
└── (pod network attachment metadata)
```
...
├── 10-flannel.conflist       [EXISTING - Flannel config, unchanged]
└── 00-multus.conf            [NEW - Generated by Multus daemon] (contains multus-shim CNI config)
```
...
| **Metrics Support**  | ✓ Supported                                 | ✗ Not available                          |
| **Features**         | Additional features via daemon              | Basic functionality only                 |
| **Resource Usage**   | Higher                                      | Lower                                    |
| **Recommendation**   | ✓ Recommended for most environments         | Legacy/resource-constrained environments |
